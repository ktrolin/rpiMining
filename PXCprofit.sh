...
echo "----------------------------------------"
echo "Checking PXC profitability..."
...
curl -s 'https://api.thingspeak.com/apps/thinghttp/send_request?api_key=QVYY69MWJKGHDS2Z' > ~/scripts/transitFiles/PXCval.txt
curl -s 'https://api.thingspeak.com/apps/thinghttp/send_request?api_key=MV8O77UVK3I0MZIX' | grep -Eo '[+-]?[0-9]+([.][0-9]+)?' > ~/scripts/transitFiles/PXCdif.txt
difficulty=( $( < ~/scripts/transitFiles/PXCdif.txt) )
conversionRate=( $( < ~/scripts/transitFiles/PXCval.txt) )
...
touch ~/scripts/transitFiles/PXCprofit.txt
echo "$netProfit1st" > ~/scripts/transitFiles/PXCprofit.txt
