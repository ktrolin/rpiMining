...
echo "----------------------------------------"
echo "Checking XMG profitability..."
...
curl -s 'https://api.thingspeak.com/apps/thinghttp/send_request?api_key=K2IMIRCBTZV2BM06' > ~/scripts/transitFiles/XMGval.txt
curl -s 'https://api.thingspeak.com/apps/thinghttp/send_request?api_key=5QA0FMXAJ1JH499A' > ~/scripts/transitFiles/XMGdif.txt
difficulty=( $( < ~/scripts/transitFiles/XMGdif.txt) )
conversionRate=( $( < ~/scripts/transitFiles/XMGval.txt) )
...
touch ~/scripts/transitFiles/XMGprofit.txt
echo "$netProfit1st" > ~/scripts/transitFiles/XMGprofit.txt
